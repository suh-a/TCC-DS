...
function openDailyReportDetails(reportId) {
  ...
  window.scrollTo({
    top: document.getElementById('report-details-section').offsetTop - 100,
    behavior: 'smooth',
  });
}

window.openDailyReportDetails = openDailyReportDetails;
...
async function getRecentsReports() {
  try {
    ...
    const response = await fetch(apiUrl(`/child/${childId}/reports/lasted`), {
      method: 'GET',
...
async function getSkillThemesList() {
  try {
...
async function loadSkillThemes() {
  try {
    skillThemes = await getSkillThemesList();
  } catch (error) {
...
function setupReportSaveButton() {
  ...
  saveButton.addEventListener('click', async function () {
    ...
    themeSelects.forEach((select, index) => {
      ...
      const score = parseInt(scoreValues[index].value, 10);
      if (themeId && !Number.isNaN(score)) {
        scores.push({
    ...
    try {
      ...
      const response = await fetch(apiUrl(`/child/${childId}/reports`), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
async function getSkillAverage() {
  try {
    const childId = localStorage.getItem('childId');
    const response = await fetch(apiUrl(`/child/${childId}/reports/avg`), {
      method: 'GET',
      headers: { 'Content-Type': 'application/json' },
    });
    return await response.json();
    ...
    const cardHTML = `
            ...
            <div class="card h-100">
              <div class="card-body">
                ...
                <div class="progress">
                  <div class="progress-bar bg-zupi-primary" role="progressbar" style="width: ${avg.average}%" aria-valuenow="${avg.average}"
