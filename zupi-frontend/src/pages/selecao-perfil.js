...
window.storeChildId = storeChildId;
...
async function loadChildrenProfiles() {
  ...
  if (!userId) {
    ...
    if (container) {
      container.innerHTML =
        '<div class="col-12"><p class="text-center text-danger">Usuário não autenticado. Faça login novamente.</p></div>';
    }
    return;
  }
  ...
  try {
    ...
    if (!response.ok) throw new Error('Erro ao buscar perfis das crianças');
    ...
    if (!Array.isArray(children) || children.length === 0) {
      container.innerHTML =
        '<div class="col-12"><p class="text-center text-muted">Nenhuma criança cadastrada. Vá para o Dashboard para adicionar.</p></div>';
      return;
    }
    ...
    if (container) {
      container.innerHTML =
        '<div class="col-12"><p class="text-center text-danger">Erro ao carregar perfis das crianças.</p></div>';
    }
  }
}
