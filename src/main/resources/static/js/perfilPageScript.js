...
document.addEventListener('DOMContentLoaded', async function () {
  console.log('[perfilPageScript] carregado');
  const childId = getQueryParam('childId') || localStorage.getItem('childId');
  console.log('[perfilPageScript] childId encontrado:', childId);
  ...
  try {
    const response = await fetch(`${API_BASE}/child/details/${childId}`, {
      method: 'GET',
      headers: {
    ...
    console.log('[perfilPageScript] resposta fetch status:', response.status);
    if (!response.ok) {
      const errorBody = await response.text();
      console.error('[perfilPageScript] erro no fetch:', errorBody);
      throw new Error('Erro ao buscar dados do perfil');
    }

    const childData = await response.json();
    console.log('[perfilPageScript] dados do perfil recebidos:', childData);
...
function renderChildProfile(child) {
  ...
  document.getElementById('childAge').textContent = child.age !== null && child.age !== undefined ? `${child.age} anos` : 'Não informado';
